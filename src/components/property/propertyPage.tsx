      ...
      >
        ...
        >
          <div className="w-full flex justify-between items-center">
            {property?.category !== undefined ? <Text size="general">{property?.category}</Text> : <></>}
            { property?.price !== undefined ? <Text size="lead" fontWeight="bold" className="text-primary">
              ${property?.price}
            </Text> : <></>}
          </div>
          {address !== undefined ? <Text size="small" className="text-black/40"> 
            {address}
          </Text> : <></>}
          {property !== undefined ? (
          ...
          {({ open }) => (
            <>
            <Disclosure.Button className={'flex justify-center items-center gap-4'}>
              <Text size="lead">Description</Text>
              <IconCaret direction={`${open ? 'up' : 'down'}`} />
            </Disclosure.Button>
            <Disclosure.Panel className=" py-8 border-b-[1px]  drop-shadow-md">
              {property.content !== undefined ? (<ContentRenderer content={property.content} />) : (
                 <Text
                  size="small"
                  fontWeight="light"
                  className=" leading-[24px] mt-4 text-black/80 "
          ...
          <div className="w-full flex flex-col gap-2 mt-2 pb-8">
            ...
            <div className="flex flex-col gap-4 mt-2">
              <div className="flex gap-2">
                <Text size="general" fontWeight="semibold">
                  Phone:{" "}
                </Text>
                <Link href={`tel:${property?.contact?.phone}`}>
                  <Text size="general">{property?.contact?.phone}</Text>
                </Link>
              </div>
              <div className="flex gap-2">
                <Text size="general" fontWeight="semibold">
                ...
                <Link href={`mailto: ${property?.contact?.email}`}>
                  <Text size="general">{property?.contact?.email}</Text>
                </Link>
              </div>
            </div>
          </div>
